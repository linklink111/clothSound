...
while window.running:
    ...
    if total_steps > 5000:
        ##########################################################################
        # 编码最后一段音轨
        sort_index = np.argsort(t_cache)   # 对t排序并取排序下表来排序pressure 
        ...
        audio = np.interp(
            np.arange(0, result_t[-1], 1/(result_t[-1]*sample_rate)), result_t, result_pressure)  # result_t[-1] * (1/sample_rate), 先前只有1/sample_rate是错误的
        
        # print(np.max(np.abs(audio)))
        # audio = audio / np.max(np.abs(audio))
        audio = audio / 5000
        audio = butter_lowpass_filter(audio, cutoff=2000, fs=sample_rate, order=6)
        wavfile.write(f"output{(sub_steps_cnt-10)//encode_gap+1}.wav", sample_rate,
            (audio * (2**15-1)).astype(np.int16))
        ############################################################################
        ############################################################################
        # 合并音轨
        ...
        # 定义采样段比例
        first_track_ratio = 2/3
        middle_track_ratio = 1/3
        last_track_ratio = 2/3

        # 循环读取每个音轨并合并
        # print(dt)  # 0.0003125
        # print((x[0,0] - listener).norm()/340 ) # 0.002751364427578235
        # 延迟声传递比dt要大，这是每个分段逐渐衰减的原因
        # 目前最简单的解决办法是不分段
        # 其次，可以重叠编码，覆盖掉断开的部分，例如 编码 a:1~15，b:5~20，c:10~25,...，然后分别取a的1~10,b的10~15，c的15~20...( 开头和结尾的取边缘10段，中间的取中间5段) 
        # 不过不同段的duration会有微小的不同，所以需要舍弃一部分（可能也不需要，因为听不出来，反正都是随机的）
        for i in range(1, sub_steps_cnt // encode_gap):
            ...
            if i == 1:
                # 对于第一个音轨，取前2/3
                segment_end = int(len(audio) * first_track_ratio)
            elif i == sub_steps_cnt // encode_gap - 1:
                # 对于最后一个音轨，取后2/3
                segment_start = int(len(audio) * (1 - last_track_ratio))
            else:
                # 对于中间的音轨，取中间1/3
                segment_start = int(len(audio) * (1 - middle_track_ratio) / 2)
                segment_end = int(len(audio) * (1 + middle_track_ratio) / 2)
            
            # 根据选择的段进行合并
            if i == 1 or i == sub_steps_cnt // encode_gap - 1:
                merged_audio = np.concatenate((merged_audio, audio[:segment_end]))
            else:
                merged_audio = np.concatenate((merged_audio, audio[segment_start:segment_end]))
    ...
    encode_gap = 5
    # 要移除的长度是 encode_gap*
    print("running,",len(pressure_cache))
    if len(t_cache) >= 13025280:  # 128*128*15*53
        print("encoding,",len(pressure_cache))
        segment_cnt += 1
        sort_index = np.argsort(t_cache)   # 对t排序并取排序下表来排序pressure 
        sorted_all_pressure = pressure_cache[sort_index]
        sorted_all_t = t_cache[sort_index]
        ...
        if target_t > 0:
            if segment_cnt == 1:
                scale = 2 * result_t[-1] / (target_t/encode_gap)
            else:
                scale = result_t[-1] / (target_t/encode_gap)
        result_t /= scale
        # if sub_steps_cnt//encode_gap == 5:
        #     # 把数据写下来分析一下为什么插值后整体逐渐衰减
        #     # 将数据写入CSV文件
        #     with open('data.csv', 'w', newline='') as csvfile:
        #         csvwriter = csv.writer(csvfile)
        #         csvwriter.writerow(['Time', 'Pressure'])  # 写入表头
        #         for t, pressure in zip(result_t, result_pressure):
        #             csvwriter.writerow([t, pressure])
        ...
        audio = audio / 5000
        audio = butter_lowpass_filter(audio, cutoff=2000, fs=sample_rate, order=6)
        wavfile.write(f"output{(sub_steps_cnt-10)//encode_gap}.wav", sample_rate,
            (audio * (2**15-1)).astype(np.int16))
        pressure_cache = pressure_cache[4341760:]
        t_cache = t_cache[4341760:]
    update_vertices()
